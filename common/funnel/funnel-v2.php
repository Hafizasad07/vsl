...
class Funnel
{
    ...
    protected function processCheckoutPage()
    {
       ...
       //$info["postData"] = $_POST
        ...
        if (isset($nameTokens[0]) && !empty($nameTokens[0])) {
            $prospectData["prospect_id"][$_POST["prospectId"]]["first_name"] = $nameTokens[0];

        }

        if (isset($nameTokens[1]) && !empty($nameTokens[1])) {
            $prospectData["prospect_id"][$_POST["prospectId"]]["last_name"] = $nameTokens[1];
       ...
       $response = $this->getRequestBody($response);
        //$response = [];
       //contact posting in AC at the spot
        $result = isset($response["response_code"]) && $response["response_code"] == 100;
    ...
    protected function processVslPage()
    {
        ...
        if (isset($_POST["email"]) && !empty($_POST["email"])) {
            ...
            if(!FreshAddress::isValidEmail($_POST["email"])){
                return $this->getNextUrl("vsl",["prospectId" => ""]);
            }
            //sending data to sticky.io
            $api_url = $this->api_url . "/api/v1/new_prospect";
            $client = new GuzzleHttp\Client([
        ...
        if ($response !== null) {
            ...
            if ($newUrl !== null) {
                ...
                if ($ptype == "vsl") {
                    ...
                    $newUrl = $this->base_url . $newUrl . "?prospectId=" . $response["prospectId"] . "&campaignId=" . $this->cId . "&email=" . base64_encode($_POST["email"]) . "&mmid=" . $mmId . "&info=" . base64_encode(json_encode($_POST)) . "&utm_campaign_id=$utm_campaign_id&h_ad_id=$h_ad_id";
                    ...
                    $newUrl = $this->base_url . $newUrl . "?customerId=" . $response["customerId"] . "&orderId=" . $response["order_id"] . "&mmid=" . $mmId . "&info=" .  $info. "&utm_campaign=$utm_campaign&h_ad_id=$h_ad_id&utm_campaign_id=$utm_campaign_id&h_ad_id=$h_ad_id";
                    return $newUrl . $funnelInfo;
                    ...
                    $newUrl = $this->base_url . $newUrl . "?customerId=" . $response["customerId"] . "&contact_id=".$response["customerId"]."&orderId=" . $_POST["orderId"] . "&mmid=" . $mmId . "&info=" . $info . "&utm_campaign=$utm_campaign&h_ad_id=$h_ad_id&utm_campaign_id=$utm_campaign_id";
                    return $newUrl . $funnelInfo;
                }
            ...
            $newUrl = $this->base_url . $newUrl . "?customerId=" . $_POST["customerId"] . "&orderId=" . $_POST["orderId"] . "&info=" . $info. "&utm_campaign=$utm_campaign&h_ad_id=$h_ad_id&utm_campaign_id=$utm_campaign_id";
            return $newUrl . $funnelInfo;
        }
    }
}
